...
def evaluate_generator():
    ...
    if args.val_orig:
        ...
        co_transform = transforms.Compose([ResizedImage3((500,500))])
    ...
    for img_id, (img, gt_mask, _, gte_mask, name) in enumerate(testloader):
        ...
        img_array = cv2.resize(img_array, (500,500), interpolation = cv2.INTER_AREA) 
...
def evaluate_discriminator():
    ...
    for img_id, (img, gt_mask, _, gte_mask, name) in enumerate(testloader):
        ...
        # Get hard prediction
        if crf == True:
            soft_pred_np = soft_pred.data.cpu().numpy()# [0]
            crf_output = np.zeros(soft_pred_np.shape)
            images = img.data.cpu().numpy().astype(np.uint8)
            for i, (image, prob_map) in enumerate(zip(images, soft_pred_np)):
                image = image.transpose(1, 2, 0)
                crf_output[i] = postprocessor(image, prob_map)
            # output = crf_output
            soft_pred_np = crf_output[0]
        else:
            soft_pred_np = soft_pred.data.cpu().numpy()[0]
        # soft_pred_np = soft_pred.data.cpu().numpy()[0]
        soft_pred_np = soft_pred_np[:,:gt_mask.shape[0],:gt_mask.shape[1]]
        hard_pred = np.argmax(soft_pred_np,axis=0).astype(np.uint8)
        # print("gen: ", hard_pred.shape)

        # Get discriminator prediction
        dis_conf = discriminator(soft_pred)
        dis_confsmax = nn.Sigmoid()(dis_conf)
        dis_soft_pred = dis_confsmax.data.cpu().numpy()[0]
        dis_hard_pred = dis_soft_pred[0]
        # plt.imshow(dis_soft_pred[0], cmap='hot', interpolation='nearest')
        sns.heatmap(dis_soft_pred[0])
        dis_hard_pred[dis_hard_pred>=0.5] = 1
        dis_hard_pred[dis_hard_pred<0.5] = 0
        print(np.unique(dis_hard_pred))
        ...
        dis_output = np.asarray(dis_hard_pred*255, dtype=np.int)
        dis_filename = os.path.join('results', '{}_dis.png'.format(name[0]))
        # dis_color_file = Image.fromarray(colorize(dis_output.transpose(1, 2, 0)), 'RGB') # 
        # dis_color_file.save(dis_filename)
        cv2.imwrite(dis_filename, dis_output)
        # plt.savefig(dis_filename)
        # plt.clf()
