...
def visual(img_array, pred, img_name, num_classes = 2):
    voc_palette = make_palette(2)
    out_im = Image.fromarray(color_seg(pred, voc_palette))
    out_im.save('results/' + img_name[0:-4] + '_out{}.png'.format(j))
    masked_im = Image.fromarray(vis_seg(img_array, out, voc_palette))
    masked_im.save('results/' + img_name[0:-4] + '_vis{}.jpg'.format(j))
