...
class FCN8s_soft(nn.Module):
    ...
    def __init__(self, n_class=2, bayes=False):
        ...
        self.drop6 = nn.Dropout2d(p=0.7)

        # fc7
        self.fc7 = nn.Conv2d(4096, 4096, 1)
        self.relu7 = nn.ReLU(inplace=True)
        self.drop7 = nn.Dropout2d(p=0.7)

        self.score_fr_cor = nn.Conv2d(4096, n_class, 1)
        self.score_pool3_cor = nn.Conv2d(256, n_class, 1)
        self.score_pool4_cor = nn.Conv2d(512, n_class, 1)

        self.upscore2_cor = nn.ConvTranspose2d(
            n_class, n_class, 4, stride=2, bias=False)
        self.upscore8_cor = nn.ConvTranspose2d(
            n_class, n_class, 16, stride=8, bias=False)
        self.upscore_pool4_cor = nn.ConvTranspose2d(
            n_class, n_class, 4, stride=2, bias=False)
    ...
    def forward(self, x):
        ...
        h = upscore2_cor + score_pool4c_cor  # 1/16
        h = self.upscore_pool4_cor(h)
        upscore_pool4_cor = h  # 1/8

        h = self.score_pool3_cor(pool3)
        h = h[:, :,
              9:9 + upscore_pool4_cor.size()[2],
              9:9 + upscore_pool4_cor.size()[3]]
        score_pool3c_cor = h  # 1/8

        h = upscore_pool4_cor + score_pool3c_cor  # 1/8

        h = self.upscore8_cor(h)
...
class FCN8s_softAtOnce(FCN8s_soft):
    ...
    def forward(self, x):
        ...
        h = self.score_pool4_cor(pool4 * 0.01)  # XXX: scaling to train at once
        h = h[:, :, 5:5 + upscore2_cor.size()[2], 5:5 + upscore2_cor.size()[3]]
        score_pool4c_cor = h  # 1/16
        ...
        h = self.score_pool3_cor(pool3 * 0.0001)  # XXX: scaling to train at once
        h = h[:, :,
              9:9 + upscore_pool4_cor.size()[2],
              9:9 + upscore_pool4_cor.size()[3]]
        score_pool3c_cor = h  # 1/8
